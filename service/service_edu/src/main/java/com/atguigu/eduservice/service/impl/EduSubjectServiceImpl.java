...
import com.atguigu.eduservice.entity.vo.dyVo;
...
public class EduSubjectServiceImpl extends ServiceImpl<EduSubjectMapper, EduSubject> implements EduSubjectService {
    ...
    private dyVo toTreeNode(EduSubject dyVo) {
        dyVo node = new dyVo();
        node.setId(dyVo.getId());
        node.setParentId(dyVo.getParentId());
        node.setTitle(dyVo.getTitle());
        return node;
    }
    @Override
    public List<dyVo> getAllSubject1() {
        List<dyVo> nodes = new ArrayList<>();
        List<EduSubject> groups  = baseMapper.selectList(null);
        for(EduSubject group: groups) {
            dyVo node = toTreeNode(group);
            dyVo parent = dyVo.findNode(nodes, group.getParentId());
            if (parent != null)
